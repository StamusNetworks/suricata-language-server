...
class TestSyntax(unittest.TestCase):
    ...
    def test_cleanrules(self):
        ...
        diags = s.analyse_file("tests/clean.rules")
    ...
    def test_invalid_multilines(self):
        s = LangServer(conn=None, settings=None)
        diags = s.analyse_file("tests/invalid-multiline.rules")
    ...
    def test_missing_incomplete(self):
        s = LangServer(conn=None, settings=None)
        diags = s.analyse_file("tests/missing-incomplete.rules")
        has_missing = 0
        for diag in diags:
            if diag.severity == 2 and diag.message.startswith("Missing closing"):
                has_missing += 1
    ...
    def test_fast_pattern_analysis(self):
        s = LangServer(conn=None, settings=None)
        diags = s.analyse_file("tests/fast-pattern-analysis.rules")
    ...
    def test_invalid_http_host(self):
        s = LangServer(conn=None, settings=None)
        diags = s.analyse_file("tests/invalid-http-host.rules")
